...
- **Mengapa `assertIn` sering lebih baik daripada `assertEqual` pada pengujian teks?**: `assertIn` memeriksa bahwa substring penting ada di dalam respons tanpa membutuhkan kecocokan persis pada seluruh isi. Ini lebih tahan terhadap perubahan format HTML, whitespace, atau tambahan konten lain; sehingga tes lebih robust. Juga respons sering berupa bytes, sehingga biasanya kita cek keberadaan substring bytes tertentu (mis. `b'Visit'`).credit)